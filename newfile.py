...
def is_pop(pop_count,day_count,open_count,is_pro,save_count):
	if pop_count < 1:#判断弹窗测试
		...
		if save_count >=10 and click_type == 2:
			print("弹出弹窗")
		else:
...
if __name__ == '__main__':
	try:
		pop_count = int(input("请输入已弹窗次数(请输入正整数):"))
		click_type = int(input("是否点击过(1.给个好评/吐槽一下  2.下次在说)"))
		day_count = int(input("请输入累计登陆天数(请输入正整数):"))
		open_count = int(input("请输入累计打开app次数(请输入正整数):"))
		is_pro = int(input("是否为付费用户(1.是 2.否):"))
		save_count = int(input("请输入保存次数(请输入正整数):"))
